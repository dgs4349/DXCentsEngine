...
static inline float  ImSaturate(float f)                                        { return (f < 0.f) ? 0.f : (f > 1.f) ? 1.f : f; }
static inline float  ImLengthSqr(const ImVec2& lhs)                             { return lhs.x*lhs.x + lhs.y*lhs.y; }
static inline float  ImLengthSqr(const ImVec4& lhs)                             { return lhs.x*lhs.x + lhs.y*lhs.y + lhs.z*lhs.z + lhs.w*lhs.w; }
static inline float  ImInvLength(const ImVec2& lhs, float fail_value)           { float d = lhs.x*lhs.x + lhs.y*lhs.y; if (d > 0.f) return 1.f / ImSqrt(d); return fail_value; }
...
struct ImGuiColumnData
{
    ...
    ImGuiColumnData()   { OffsetNorm = OffsetNormBeforeResize = 0.f; Flags = 0; }
};
...
struct ImGuiNextWindowData
{
    ...
    ImGuiNextWindowData()
    {
        ...
        PosVal = PosPivotVal = SizeVal = ImVec2(0.f, 0.f);
        ContentSizeVal = ImVec2(0.f, 0.f);
        CollapsedVal = false;
        SizeConstraintRect = ImRect();
        SizeCallback = NULL;
        SizeCallbackUserData = NULL;
        BgAlphaVal = FLT_MAX;
        MenuBarOffsetMinVal = ImVec2(0.f, 0.f);
    }
...
struct ImGuiContext
{
    ...
    ImGuiWindow*            NavWindowingTargetAnim;             // Record of last valid NavWindowingTarget until DimBgRatio and NavWindowingHighlightAlpha becomes 0.f
    ...
    float                   DragSpeedDefaultRatio;              // If speed == 0.f, uses (max-min) * DragSpeedDefaultRatio
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas) : BackgroundDrawList(NULL), ForegroundDrawList(NULL)
    {
        ...
        HoveredIdTimer = HoveredIdNotActiveTimer = 0.f;
        ActiveId = 0;
        ...
        LastActiveId = 0;
        LastActiveIdTimer = 0.f;
        LastValidMousePos = ImVec2(0.f, 0.f);
        MovingWindow = NULL;
        ...
        NavWindowingTimer = NavWindowingHighlightAlpha = 0.f;
        ...
        DragDropAcceptIdCurrRectSurface = 0.f;
        ...
        DragCurrentAccum = 0.f;
        DragSpeedDefaultRatio = 1.f / 100.f;
        ScrollbarClickDeltaToGrabCenter = ImVec2(0.f, 0.f);
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    bool                    NavHasScroll;           // Set when scrolling can be used (ScrollMax > 0.f)
    ...
    float                   TextWrapPos;            // == TextWrapPosStack.back() [empty == -1.f]
    ...
    ImGuiWindowTempData()
    {
        CursorPos = CursorPosPrevLine = CursorStartPos = CursorMaxPos = ImVec2(0.f, 0.f);
        CurrentLineSize = PrevLineSize = ImVec2(0.f, 0.f);
        CurrentLineTextBaseOffset = PrevLineTextBaseOffset = 0.f;
        TreeDepth = 0;
        ...
        Indent = ImVec1(0.f);
        GroupOffset = ImVec1(0.f);
        ColumnsOffset = ImVec1(0.f);
        ColumnsSet = NULL;
    }
};
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  Scroll;
    ImVec2                  ScrollTarget;                       // target scroll position. stored as cursor position with scrolling canceled out, so the highest point is always 0.f. (FLT_MAX for no change)
    ImVec2                  ScrollTargetCenterRatio;            // 0.f = scroll so that target position is at top, 0.5f = scroll so that target position is centered
...
public:
    ...
    float       TitleBarHeight() const                  { return (Flags & ImGuiWindowFlags_NoTitleBar) ? 0.f : CalcFontSize() + GImGui->Style.FramePadding.y * 2.0f; }
    ImRect      TitleBarRect() const                    { return ImRect(Pos, ImVec2(Pos.x + SizeFull.x, Pos.y + TitleBarHeight())); }
    float       MenuBarHeight() const                   { return (Flags & ImGuiWindowFlags_MenuBar) ? DC.MenuBarOffset.y + CalcFontSize() + GImGui->Style.FramePadding.y * 2.0f : 0.f; }
...
struct ImGuiTabItem
{
    ...
    ImGuiTabItem()      { ID = Flags = 0; LastFrameVisible = LastFrameSelected = -1; NameOffset = -1; Offset = Width = WidthContents = 0.f; }
};
...
namespace ImGui
{
    ...
    IMGUI_API void          ItemSize(const ImVec2& size, float text_offset_y = 0.f);
    IMGUI_API void          ItemSize(const ImRect& bb, float text_offset_y = 0.f);
    ...
    IMGUI_API void          PushMultiItemsWidths(int components, float width_full = 0.f);
    ...
    IMGUI_API ImVec2        GetNavInputAmount2d(ImGuiNavDirSourceFlags dir_sources, ImGuiInputReadMode mode, float slow_factor = 0.f, float fast_factor = 0.f);
    ...
    inline bool             IsNavInputDown(ImGuiNavInput n)                             { return GImGui->IO.NavInputs[n] > 0.f; }
    inline bool             IsNavInputPressed(ImGuiNavInput n, ImGuiInputReadMode mode) { return GetNavInputAmount(n, mode) > 0.f; }
    inline bool             IsNavInputPressedAnyOfTwo(ImGuiNavInput n1, ImGuiNavInput n2, ImGuiInputReadMode mode) { return (GetNavInputAmount(n1, mode) + GetNavInputAmount(n2, mode)) > 0.f; }
    ...
    IMGUI_API void          RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border = true, float rounding = 0.f);
    IMGUI_API void          RenderFrameBorder(ImVec2 p_min, ImVec2 p_max, float rounding = 0.f);
    IMGUI_API void          RenderColorRectWithAlphaCheckerboard(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, float grid_step, ImVec2 grid_off, float rounding = 0.f, int rounding_corners_flags = ~0);
    IMGUI_API void          RenderArrow(ImVec2 pos, ImGuiDir dir, float scale = 1.f);
    ...
    IMGUI_API bool          SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend = 0.f, float hover_visibility_delay = 0.f);
