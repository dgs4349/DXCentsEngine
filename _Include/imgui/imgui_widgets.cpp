...
#pragma clang diagnostic ignored "-Wfloat-equal"            // warning : comparing floating point with == or != is unsafe   // storing and comparing against same constants (typically 0.f) is ok.
...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    const bool wrap_enabled = (wrap_pos_x >= 0.f);
    ...
    {
        const float wrap_width = wrap_enabled ? CalcWrapWidthForPos(window->DC.CursorPos, wrap_pos_x) : 0.f;
...
{
    bool need_backup = (GImGui->CurrentWindow->DC.TextWrapPos < 0.f);  // Keep existing wrap position if one is already set
    if (need_backup)
        PushTextWrapPos(0.f);
    TextV(fmt, args);
    if (need_backup)
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + (label_size.x > 0.f ? style.ItemInnerSpacing.x : 0.f), style.FramePadding.y*2) + label_size);
    ...
    RenderTextClipped(value_bb.Min, value_bb.Max, value_text_begin, value_text_end, NULL, ImVec2(0.f,0.5f));
    if (label_size.x > 0.f)
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const float text_base_offset_y = ImMax(0.f, window->DC.CurrentLineTextBaseOffset); // Latch before ItemSize changes it
    const float line_height = ImMax(ImMin(window->DC.CurrentLineSize.y, g.FontSize + g.Style.FramePadding.y*2), g.FontSize);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(g.FontSize + (label_size.x > 0.f ? (label_size.x + style.FramePadding.x*2) : 0.f), ImMax(line_height, label_size.y)));  // Empty text doesn't add padding
    ItemSize(bb);
    if (!ItemAdd(bb, 0))
        return;
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            ...
            if ((flags & ImGuiButtonFlags_Repeat) && g.ActiveId == id && g.IO.MouseDownDuration[0] > 0.f && IsMouseClicked(0, true))
                pressed = true;
        }
...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg)
{
    ...
    IM_ASSERT(size_arg.x != 0.f && size_arg.y != 0.f);

    const ImGuiID id = window->GetID(str_id);
    ImVec2 size = CalcItemSize(size_arg, 0.f, 0.f);
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ...
    ItemSize(bb, (size.y >= default_size) ? g.Style.FramePadding.y : 0.f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    RenderArrow(bb.Min + ImVec2(ImMax(0.f, (size.x - g.FontSize) * 0.5f), ImMax(0.f, (size.y - g.FontSize) * 0.5f)), dir);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius)
{
    ...
    float cross_extent = (radius * 0.7071f) - 1.f;
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    if (hovered || held)
        window->DrawList->AddCircleFilled(bb.GetCenter() + ImVec2(0.f, -0.5f), g.FontSize * 0.5f + 1.f, col, 9);
    RenderArrow(bb.Min + g.Style.FramePadding, window->Collapsed ? ImGuiDir_Right : ImGuiDir_Down, 1.f);
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    float other_scrollbar_size_w = other_scrollbar ? style.ScrollbarSize : 0.f;
    ...
    if (!horizontal)
        bb.Min.y += window->TitleBarHeight() + ((window->Flags & ImGuiWindowFlags_MenuBar) ? window->MenuBarHeight() : 0.f);

    const float bb_height = bb.GetHeight();
    if (bb.GetWidth() <= 0.f || bb_height <= 0.f)
        return;
    ...
    const bool allow_interaction = (alpha >= 1.f);
    ...
    bb.Expand(ImVec2(-ImClamp((float)(int)((bb.Max.x - bb.Min.x - 2.0f) * 0.5f), 0.f, 3.0f), -ImClamp((float)(int)((bb.Max.y - bb.Min.y - 2.0f) * 0.5f), 0.f, 3.0f)));
    ...
    IM_ASSERT(ImMax(win_size_contents_v, win_size_avail_v) > 0.f); // Adding this assert to check if the ImMax(XXX,1.f) is still needed. PLEASE CONTACT ME if this triggers.
    const float win_size_v = ImMax(ImMax(win_size_contents_v, win_size_avail_v), 1.f);
    ...
    float scroll_max = ImMax(1.f, win_size_contents_v - win_size_avail_v);
    float scroll_ratio = ImSaturate(scroll_v / scroll_max);
    float grab_v_norm = scroll_ratio * (scrollbar_size_v - grab_h_pixels) / scrollbar_size_v;
    if (held && allow_interaction && grab_h_norm < 1.f)
    {
        ...
        if (!previously_held)
        {
            ...
            {
                seek_absolute = true;
                *click_delta_to_grab_center_v = 0.f;
            }
        }

        // Apply scroll
        // It is ok to modify Scroll here because we are being called in Begin() after the calculation of SizeContents and before setting up our starting position
        const float scroll_v_norm = ImSaturate((clicked_v_norm - *click_delta_to_grab_center_v - grab_h_norm*0.5f) / (1.f - grab_h_norm));
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ...
    if (border_col.w > 0.f)
    {
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(border_col), 0.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.f ? style.ItemInnerSpacing.x + label_size.x : 0.f), label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (*v)
    {
        const float pad = ImMax(1.f, (float)(int)(square_sz / 6.0f));
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    const float radius = (square_sz - 1.f) * 0.5f;
...
// size_arg (for each axis) < 0.f: align to end, 0.f: auto, > 0.f: specified size
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    RenderRectFilledRangeH(window->DrawList, bb, GetColorU32(ImGuiCol_PlotHistogram), 0.f, fraction, style.FrameRounding);
    ...
    if (overlay_size.x > 0.f)
        RenderTextClipped(ImVec2(ImClamp(fill_br.x + style.ItemSpacing.x, bb.Min.x, bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x), bb.Min.y), bb.Max, overlay, NULL, &overlay_size, ImVec2(0.f,0.5f), &bb);
}
...
void ImGui::NewLine()
{
    ...
    if (window->DC.CurrentLineSize.y > 0.f)     // In the event that we are on a line with items that is smaller that FontSize high, we will preserve its height.
        ItemSize(ImVec2(0,0));
    else
        ItemSize(ImVec2(0.f, g.FontSize));
...
void ImGui::Separator()
{
    ...
    const ImRect bb(ImVec2(x1, window->DC.CursorPos.y), ImVec2(x2, window->DC.CursorPos.y+1.f));
...
void ImGui::VerticalSeparator()
{
    ...
    const ImRect bb(ImVec2(window->DC.CursorPos.x, y1), ImVec2(window->DC.CursorPos.x + 1.f, y2));
    ItemSize(ImVec2(bb.GetWidth(), 0.f));
    if (!ItemAdd(bb, 0))
        return;
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay)
{
    ...
    bb_interact.Expand(axis == ImGuiAxis_Y ? ImVec2(0.f, hover_extend) : ImVec2(hover_extend, 0.f));
    ...
    if (held)
    {
        ...
        // Minimum pane size
        float size_1_maximum_delta = ImMax(0.f, *size1 - min_size1);
        float size_2_maximum_delta = ImMax(0.f, *size2 - min_size2);
        ...
        if (mouse_delta != 0.f)
        {
            ...
            *size1 += mouse_delta;
            *size2 -= mouse_delta;
            bb_render.Translate((axis == ImGuiAxis_X) ? ImVec2(mouse_delta, 0.f) : ImVec2(0.f, mouse_delta));
            MarkItemEdited(id);
        }
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float arrow_size = (flags & ImGuiComboFlags_NoArrowButton) ? 0.f : GetFrameHeight();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ...
    const ImRect value_bb(frame_bb.Min, frame_bb.Max - ImVec2(arrow_size, 0.f));
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, value_bb.Max, preview_value, NULL, NULL, ImVec2(0.f,0.f));
    if (label_size.x > 0)
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
...
static bool DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* format)
{
    ...
    if (data_type == ImGuiDataType_S32)
    {
        ...
        else if (op == '/') { if (sscanf(buf, "%f", &arg1f) && arg1f != 0.f) *v = (int)(arg0i / arg1f); }  // Divide
    ...
    else if (data_type == ImGuiDataType_Float)
    {
        ...
        else if (op == '/') { if (arg1f != 0.f) *v = arg0f / arg1f; } // Divide
...
{
    static const float min_steps[10] = { 1.f, 0.1f, 0.01f, 0.001f, 0.0001f, 0.00001f, 0.000001f, 0.0000001f, 0.00000001f, 0.000000001f };
    if (decimal_precision < 0)
        return FLT_MIN;
    return (decimal_precision < IM_ARRAYSIZE(min_steps)) ? min_steps[decimal_precision] : ImPow(10.f, (float)-decimal_precision);
}
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiDragFlags flags)
{
    ...
    const bool is_power = (power != 1.f && is_decimal && has_min_max && (v_max - v_min < FLT_MAX));

    // Default tweak speed
    if (v_speed == 0.f && has_min_max && (v_max - v_min < FLT_MAX))
        v_speed = (float)((v_max - v_min) * g.DragSpeedDefaultRatio);

    // Inputs accumulates into g.DragCurrentAccum, which is flushed into the current value as soon as it makes a difference with our precision settings
    float adjust_delta = 0.f;
    if (g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() && g.IO.MouseDragMaxDistanceSqr[0] > 1.f*1.f)
    {
        adjust_delta = g.IO.MouseDelta[axis];
        if (g.IO.KeyAlt)
            adjust_delta *= 1.f / 100.f;
        if (g.IO.KeyShift)
            adjust_delta *= 10.f;
    }
    else if (g.ActiveIdSource == ImGuiInputSource_Nav)
    {
        ...
        adjust_delta = GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad, ImGuiInputReadMode_RepeatFast, 1.f / 10.f, 10.f)[axis];
    ...
    bool is_already_past_limits_and_pushing_outward = has_min_max && ((*v >= v_max && adjust_delta > 0.f) || (*v <= v_min && adjust_delta < 0.f));
    ...
    TYPE v_cur = *v;
    FLOATTYPE v_old_ref_for_accum_remainder = (FLOATTYPE)0.f;

    if (is_power)
    {
        // Offset + round to user desired precision, with a curve on the v_min..v_max range to get more precision on one side of the range
        FLOATTYPE v_old_norm_curved = ImPow((FLOATTYPE)(v_cur - v_min) / (FLOATTYPE)(v_max - v_min), (FLOATTYPE)1.f / power);
    ...
    if (is_power)
    {
        FLOATTYPE v_cur_norm_curved = ImPow((FLOATTYPE)(v_cur - v_min) / (FLOATTYPE)(v_max - v_min), (FLOATTYPE)1.f / power);
    ...
    {
        if (v_cur < v_min || (v_cur > *v && adjust_delta < 0.f && !is_decimal))
            v_cur = v_min;
        if (v_cur > v_max || (v_cur < *v && adjust_delta > 0.f && !is_decimal))
            v_cur = v_max;
    }
...
float ImGui::SliderCalcRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, float power, float linear_zero_pos)
{
    ...
    const bool is_power = (power != 1.f) && (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double);
    const TYPE v_clamped = (v_min < v_max) ? ImClamp(v, v_min, v_max) : ImClamp(v, v_max, v_min);
    if (is_power)
    {
        if (v_clamped < 0.f)
        {
            const float f = 1.f - (float)((v_clamped - v_min) / (ImMin((TYPE)0, v_max) - v_min));
            return (1.f - ImPow(f, 1.f/power)) * linear_zero_pos;
        }
        else
        {
            const float f = (float)((v_clamped - ImMax((TYPE)0, v_min)) / (v_max - ImMax((TYPE)0, v_min)));
            return linear_zero_pos + ImPow(f, 1.f/power) * (1.f - linear_zero_pos);
        }
    }
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const bool is_decimal = (data_type == ImGuiDataType_Float) || (data_type == ImGuiDataType_Double);
    const bool is_power = (power != 1.f) && is_decimal;
    ...
    // For power curve sliders that cross over sign boundary we want the curve to be symmetric around 0.f
    float linear_zero_pos;   // 0.0->1.f
    if (is_power && v_min * v_max < 0.f)
    {
        // Different sign
        const FLOATTYPE linear_dist_min_to_0 = ImPow(v_min >= 0 ? (FLOATTYPE)v_min : -(FLOATTYPE)v_min, (FLOATTYPE)1.f/power);
        const FLOATTYPE linear_dist_max_to_0 = ImPow(v_max >= 0 ? (FLOATTYPE)v_max : -(FLOATTYPE)v_max, (FLOATTYPE)1.f/power);
        linear_zero_pos = (float)(linear_dist_min_to_0 / (linear_dist_min_to_0 + linear_dist_max_to_0));
    }
    else
    {
        // Same sign
        linear_zero_pos = v_min < 0.f ? 1.f : 0.f;
    }
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            else
            {
                ...
                clicked_t = (slider_usable_sz > 0.f) ? ImClamp((mouse_abs_pos - slider_usable_pos_min) / slider_usable_sz, 0.f, 1.f) : 0.f;
                if (axis == ImGuiAxis_Y)
                    clicked_t = 1.f - clicked_t;
                set_new_value = true;
            }
        }
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            const ImVec2 delta2 = GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad, ImGuiInputReadMode_RepeatFast, 0.f, 0.f);
            ...
            else if (delta != 0.f)
            {
                ...
                {
                    delta /= 100.f;    // Gamepad/keyboard tweak speeds in % of slider bounds
                    if (IsNavInputDown(ImGuiNavInput_TweakSlow))
                        delta /= 10.f;
                }
                else
                {
                    if ((v_range >= -100.f && v_range <= 100.f) || IsNavInputDown(ImGuiNavInput_TweakSlow))
                        delta = ((delta < 0.f) ? -1.f : +1.f) / (float)v_range; // Gamepad/keyboard tweak speeds in integer steps
                    else
                        delta /= 100.f;
                }
                if (IsNavInputDown(ImGuiNavInput_TweakFast))
                    delta *= 10.f;
                set_new_value = true;
                if ((clicked_t >= 1.f && delta > 0.f) || (clicked_t <= 0.f && delta < 0.f)) // This is to avoid applying the saturation when already past the limits
                    set_new_value = false;
                else
        ...
        if (set_new_value)
        {
            ...
            if (is_power)
            {
                ...
                if (clicked_t < linear_zero_pos)
                {
                    ...
                    float a = 1.f - (clicked_t / linear_zero_pos);
                    a = ImPow(a, power);
                ...
                else
                {
                    ...
                    float a;
                    if (ImFabs(linear_zero_pos - 1.f) > 1.e-6f)
                        a = (clicked_t - linear_zero_pos) / (1.f - linear_zero_pos);
                    else
                        a = clicked_t;
                    a = ImPow(a, power);
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max, const char* format)
{
    ...
    bool value_changed = SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, format, 1.f);
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImRect bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.f ? style.ItemInnerSpacing.x + label_size.x : 0.f, 0.f));
    ...
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
    RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f,0.f));
    if (label_size.x > 0.f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* data_ptr, const void* step, const void* step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (step != NULL)
    {
        ...
        PushID(label);
        PushItemWidth(ImMax(1.f, CalcItemWidth() - (button_size + style.ItemInnerSpacing.x) * 2));
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    return InputScalar(label, ImGuiDataType_Float, (void*)v, (void*)(step>0.f ? &step : NULL), (void*)(step_fast>0.f ? &step_fast : NULL), format, flags);
}
...
static ImVec2 InputTextCalcTextSizeW(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ...
    if (line_width > 0 || text_size.y == 0.f)                        // whereas size.y will ignore the trailing \n
        text_size.y += line_height;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (io.MouseDown[0] && !state->SelectedAllMouseLock && (io.MouseDelta.x != 0.f || io.MouseDelta.y != 0.f))
        {
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (IsKeyPressedMap(ImGuiKey_UpArrow) && is_multiline)     { if (io.KeyCtrl) SetWindowScrollY(draw_window, ImMax(draw_window->Scroll.y - g.FontSize, 0.f)); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll))
            {
                ...
                if (cursor_offset.x < state->ScrollX)
                    state->ScrollX = (float)(int)ImMax(0.f, cursor_offset.x - scroll_increment_x);
            ...
            if (is_multiline)
            {
                ...
                if (cursor_offset.y - g.FontSize < scroll_y)
                    scroll_y = ImMax(0.f, cursor_offset.y - g.FontSize);
        ...
        // Draw selection
        const ImVec2 draw_scroll = ImVec2(state->ScrollX, 0.f);
        if (render_selection)
        {
            const ImWchar* text_selected_begin = text_begin + ImMin(state->Stb.select_start, state->Stb.select_end);
            const ImWchar* text_selected_end = text_begin + ImMax(state->Stb.select_start, state->Stb.select_end);

            ImU32 bg_color = GetColorU32(ImGuiCol_TextSelectedBg, render_cursor ? 1.f : 0.6f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            ...
            float bg_offy_dn = is_multiline ? 0.f : 2.0f;
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect_size.x <= 0.f) rect_size.x = (float)(int)(g.Font->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty lines
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (state->CursorAnim <= 0.f) || ImFmod(state->CursorAnim, 1.20f) <= 0.80f;
            ImVec2 cursor_screen_pos = draw_pos + cursor_offset - draw_scroll;
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - g.FontSize + 0.5f, cursor_screen_pos.x + 1.f, cursor_screen_pos.y - 1.5f);
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32(ImGuiCol_Text));

            // Notify OS of text input position for advanced IME (-1 x offset so that Windows IME can cover our cursor. Bit of an extra nicety.)
            if (!is_readonly)
                g.PlatformImePos = ImVec2(cursor_screen_pos.x - 1.f, cursor_screen_pos.y - g.FontSize);
        }
    }
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, col, buf_display, buf_display_end, 0.f, is_multiline ? NULL : &clip_rect);
        }
    }

    if (is_multiline)
    {
        Dummy(text_size + ImVec2(0.f, g.FontSize)); // Always add room to scroll an extra line
        EndChildFrame();
        EndGroup();
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const float square_sz = GetFrameHeight();
    const float w_extra = (flags & ImGuiColorEditFlags_NoSmallPreview) ? 0.f : (square_sz + style.ItemInnerSpacing.x);
    ...
    float f[4] = { col[0], col[1], col[2], alpha ? col[3] : 1.f };
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const float w_item_one  = ImMax(1.f, (float)(int)((w_items_all - (style.ItemInnerSpacing.x) * (components-1)) / (float)components));
        const float w_item_last = ImMax(1.f, (float)(int)(w_items_all - (w_item_one + style.ItemInnerSpacing.x) * (components-1)));
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            {
                value_changed |= DragFloat(ids[n], &f[n], 1.f/255.0f, 0.f, hdr ? 0.f : 1.f, fmt_table_float[fmt_idx][n]);
                value_changed_as_float |= value_changed;
            }
            else
            {
                value_changed |= DragInt(ids[n], &i[n], 1.f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][n]);
            }
            if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.f);
...
{
    float col4[4] = { col[0], col[1], col[2], 1.f };
    ...
    if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF)
    {
        ...
        for (float y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++)
        {
            ...
            for (float x = p_min.x + grid_off.x + (yi & 1) * grid_step; x < p_max.x; x += grid_step * 2.0f)
            {
                ...
                window->DrawList->AddRectFilled(ImVec2(x1,y1), ImVec2(x2,y2), col_bg2, rounding_corners_flags_cell ? rounding : 0.f, rounding_corners_flags_cell);
            }
        }
    }
    else
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 triangle_pa = ImVec2(triangle_r, 0.f); // Hue point.
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive())
        {
            ...
            if (ImTriangleContainsPoint(triangle_pa, triangle_pb, triangle_pc, ImRotate(initial_off, cos_hue_angle, sin_hue_angle)))
            {
                ...
                V = ImClamp(1.f - vv, 0.0001f, 1.f);
                S = ImClamp(uu / V, 0.0001f, 1.f);
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        if (IsItemActive())
        {
            ...
            V = 1.f - ImSaturate((io.MousePos.y - picker_pos.y) / (sv_picker_size-1));
            value_changed = value_changed_sv = true;
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context");
    ...
    if (alpha_bar)
    {
        ...
        if (IsItemActive())
        {
            col[3] = 1.f - ImSaturate((io.MousePos.y - picker_pos.y) / (sv_picker_size-1));
            value_changed = true;
        }
    }
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ImVec4 col_v4(col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.f : col[3]);
        if ((flags & ImGuiColorEditFlags_NoLabel))
            Text("Current");
        ...
        {
            Text("Original");
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.f : ref_col[3]);
    ...
    if (value_changed_h || value_changed_sv)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ColorConvertHSVtoRGB(H >= 1.f ? H - 10 * 1e-6f : H, S > 0.f ? S : 10*1e-6f, V > 0.f ? V : 1e-6f, col[0], col[1], col[2]);
        }
        else if (flags & ImGuiColorEditFlags_InputHSV)
        {
            col[0] = H;
    ...
    ImU32 col32_no_alpha = ColorConvertFloat4ToU32(ImVec4(R, G, B, 1.f));
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int n = 0; n < 6; n++)
        {
            ...
            const float a1 = (n+1.f)/6.0f * 2.0f * IM_PI + aeps;
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        RenderFrameBorder(picker_pos, picker_pos + ImVec2(sv_picker_size,sv_picker_size), 0.f);
        ...
        RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), 0.f);
        RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f);
    }

    // Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    float sv_cursor_rad = value_changed_sv ? 10.f : 6.0f;
    ...
    if (alpha_bar)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(bar1_bb.Min, bar1_bb.Max, IM_COL32(0,0,0,0), bar1_bb.GetWidth() / 2.0f, ImVec2(0.f, 0.f));
        draw_list->AddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max, col32_no_alpha, col32_no_alpha, col32_no_alpha & ~IM_COL32_A_MASK, col32_no_alpha & ~IM_COL32_A_MASK);
        float bar1_line_y = (float)(int)(picker_pos.y + (1.f - alpha) * sv_picker_size + 0.5f);
        RenderFrameBorder(bar1_bb.Min, bar1_bb.Max, 0.f);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    ImVec4 col_rgb_without_alpha(col_rgb.x, col_rgb.y, col_rgb.z, 1.f);
    float grid_step = ImMin(size.x, size.y) / 2.99f;
    float rounding = ImMin(g.Style.FrameRounding, grid_step * 0.5f);
    ImRect bb_inner = bb;
    float off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This offset seemed like a good middle ground to reduce those artifacts.
    bb_inner.Expand(off);
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.f)
    {
    ...
    RenderNavHighlight(bb, id);
    if (g.Style.FrameBorderSize > 0.f)
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ImVec4 cf(col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.f : col[3]);
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (BeginPopup("Copy"))
    {
        int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
        char buf[64];
        ImFormatString(buf, IM_ARRAYSIZE(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)", col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.f : col[3]);
        if (Selectable(buf))
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ImVec2 picker_size(g.FontSize * 8, ImMax(g.FontSize * 8 - (GetFrameHeight() + g.Style.ItemInnerSpacing.x), 1.f)); // FIXME: Picker size copied from main picker function
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const ImVec2 padding = (display_frame || (flags & ImGuiTreeNodeFlags_FramePadding)) ? style.FramePadding : ImVec2(style.FramePadding.x, 0.f);
    ...
    const float text_width = g.FontSize + (label_size.x > 0.f ? label_size.x + padding.x*2 : 0.f);   // Include collapser
    ...
    if (display_frame)
    {
        ...
        RenderArrow(frame_bb.Min + ImVec2(padding.x, text_base_offset_y), is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.f);
        if (g.LogEnabled)
        {
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 size(size_arg.x != 0.f ? size_arg.x : label_size.x, size_arg.y != 0.f ? size_arg.y : label_size.y);
    ImVec2 pos = window->DC.CursorPos;
    ...
    ImVec2 size_draw((size_arg.x != 0 && !(flags & ImGuiSelectableFlags_DrawFillAvailWidth)) ? size_arg.x : w_draw, size_arg.y != 0.f ? size_arg.y : size.y);
    ImRect bb(pos, pos + size_draw);
    if (size_arg.x == 0.f || (flags & ImGuiSelectableFlags_DrawFillAvailWidth))
    ...
    {
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
        RenderFrame(bb.Min, bb.Max, col, false, 0.f);
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    ImRect bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.f ? style.ItemInnerSpacing.x + label_size.x : 0.f, 0.f));
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
    ...
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.f ? style.ItemInnerSpacing.x + label_size.x : 0.f, 0));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (values_count >= values_count_min)
    {
        ...
        const float inv_scale = (scale_min == scale_max) ? 0.f : (1.f / (scale_max - scale_min));

        float v0 = values_getter(data, (0 + values_offset) % values_count);
        float t0 = 0.f;
        ImVec2 tp0 = ImVec2( t0, 1.f - ImSaturate((v0 - scale_min) * inv_scale) );                       // Point in the normalized space of our target rectangle
        float histogram_zero_line_t = (scale_min * scale_max < 0.f) ? (-scale_min * inv_scale) : (scale_min < 0.f ? 0.f : 1.f);   // Where does the zero line stands
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            const ImVec2 tp1 = ImVec2( t1, 1.f - ImSaturate((v1 - scale_min) * inv_scale) );
    ...
    // Text overlay
    if (overlay_text)
        RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max, overlay_text, NULL, NULL, ImVec2(0.5f,0.f));
...
{
    return ImMax(0.f, avail_w - Width);
}
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        popup_pos = ImVec2(pos.x - 1.f - (float)(int)(style.ItemSpacing.x * 0.5f), pos.y - style.FramePadding.y + window->MenuBarHeight());
        window->DC.CursorPos.x += (float)(int)(style.ItemSpacing.x * 0.5f);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, style.ItemSpacing * 2.0f);
        float w = label_size.x;
        pressed = Selectable(label, menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_PressedOnClick | ImGuiSelectableFlags_DontClosePopups | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.f));
        PopStyleVar();
        window->DC.CursorPos.x += (float)(int)(style.ItemSpacing.x * (-1.f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
    }
    else
    {
        // Menu inside a menu
        popup_pos = ImVec2(pos.x, pos.y - style.WindowPadding.y);
        float w = window->MenuColumns.DeclColumns(label_size.x, 0.f, (float)(int)(g.FontSize * 1.20f)); // Feedback to next frame
        float extra_w = ImMax(0.f, GetContentRegionAvail().x - w);
        pressed = Selectable(label, menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_PressedOnClick | ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_DrawFillAvailWidth | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.f));
        if (!enabled) PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
        RenderArrow(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.30f, 0.f), ImGuiDir_Right);
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && g.OpenPopupStack.Size > g.BeginPopupStack.Size && g.OpenPopupStack[g.BeginPopupStack.Size].ParentWindow == window && !(window->Flags & ImGuiWindowFlags_MenuBar))
        {
            if (ImGuiWindow* next_window = g.OpenPopupStack[g.BeginPopupStack.Size].Window)
            {
                ...
                float extra = ImClamp(ImFabs(ta.x - tb.x) * 0.30f, 5.0f, 30.f); // add a bit of extra slack.
                ta.x += (window->Pos.x < next_window->Pos.x) ? -0.5f : +0.5f;    // to avoid numerical issues
                tb.y = ta.y + ImMax((tb.y - extra) - ta.y, -100.f);             // triangle is maximum 200 high to limit the slope and the bias toward large sub-menus // FIXME: Multiply by fb_scale?
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        window->DC.CursorPos.x += (float)(int)(style.ItemSpacing.x * 0.5f);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, style.ItemSpacing * 2.0f);
        pressed = Selectable(label, false, flags, ImVec2(w, 0.f));
        PopStyleVar();
        window->DC.CursorPos.x += (float)(int)(style.ItemSpacing.x * (-1.f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
    }
    else
    {
        ...
        float extra_w = ImMax(0.f, GetContentRegionAvail().x - w);
        pressed = Selectable(label, false, flags | ImGuiSelectableFlags_DrawFillAvailWidth, ImVec2(w, 0.f));
...
ImGuiTabBar::ImGuiTabBar()
{
    ...
    ContentsHeight = 0.f;
    OffsetMax = OffsetNextTab = 0.f;
    ScrollingAnim = ScrollingTarget = ScrollingTargetDistToVisibility = ScrollingSpeed = 0.f;
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    const float y = tab_bar->BarRect.Max.y - 1.f;
    {
        ...
        window->DrawList->AddLine(ImVec2(separator_min_x, y), ImVec2(separator_max_x, y), col, 1.f);
    }
    return true;
}
...
void    ImGui::EndTabBar()
{
    ...
    if (tab_bar->VisibleTabWasSubmitted || tab_bar->VisibleTabId == 0 || tab_bar_appearing)
        tab_bar->ContentsHeight = ImMax(window->DC.CursorPos.y - tab_bar->BarRect.Max.y, 0.f);
    else
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    float width_total_contents = 0.f;
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        width_total_contents += (tab_n > 0 ? g.Style.ItemInnerSpacing.x : 0.f) + tab->WidthContents;
    ...
    float width_excess = (width_avail < width_total_contents) ? (width_total_contents - width_avail) : 0.f;
    if (width_excess > 0.f && (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown))
    {
        ...
        while (width_excess > 0.f && tab_count_same_width < tab_bar->Tabs.Size)
        {
            ...
            float width_to_remove_per_tab_max = (tab_count_same_width < tab_bar->Tabs.Size) ? (width_sort_buffer[0].Width - width_sort_buffer[tab_count_same_width].Width) : (width_sort_buffer[0].Width - 1.f);
    ...
    tab_bar->OffsetMax = ImMax(offset_x - g.Style.ItemInnerSpacing.x, 0.f);
    tab_bar->OffsetNextTab = 0.f;
    ...
    if (tab_bar->ScrollingAnim != tab_bar->ScrollingTarget)
    {
        ...
        tab_bar->ScrollingSpeed = ImMax(tab_bar->ScrollingSpeed, 70.f * g.FontSize);
        tab_bar->ScrollingSpeed = ImMax(tab_bar->ScrollingSpeed, ImFabs(tab_bar->ScrollingTarget - tab_bar->ScrollingAnim) / 0.3f);
        const bool teleport = (tab_bar->PrevFrameVisible + 1 < g.FrameCount) || (tab_bar->ScrollingTargetDistToVisibility > 10.f * g.FontSize);
        tab_bar->ScrollingAnim = teleport ? tab_bar->ScrollingTarget : ImLinearSweep(tab_bar->ScrollingAnim, tab_bar->ScrollingTarget, g.IO.DeltaTime * tab_bar->ScrollingSpeed);
    }
    else
    {
        tab_bar->ScrollingSpeed = 0.f;
    }
...
static float ImGui::TabBarScrollClamp(ImGuiTabBar* tab_bar, float scrolling)
{
    ...
    return ImMax(scrolling, 0.f);
}

static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    ImGuiContext& g = *GImGui;
    float margin = g.FontSize * 1.f; // When to scroll to make Tab N+1 visible always make a bit of N visible to suggest more scrolling area (since we don't have a scrollbar)
    int order = tab_bar->GetTabOrder(tab);
    float tab_x1 = tab->Offset + (order > 0 ? -margin : 0.f);
    float tab_x2 = tab->Offset + tab->Width + (order + 1 < tab_bar->Tabs.Size ? margin : 1.f);
    tab_bar->ScrollingTargetDistToVisibility = 0.f;
    if (tab_bar->ScrollingTarget > tab_x1)
    {
        tab_bar->ScrollingTargetDistToVisibility = ImMax(tab_bar->ScrollingAnim - tab_x2, 0.f);
        tab_bar->ScrollingTarget = tab_x1;
    }
    else if (tab_bar->ScrollingTarget < tab_x2 - tab_bar->BarRect.GetWidth())
    {
        tab_bar->ScrollingTargetDistToVisibility = ImMax((tab_x1 - tab_bar->BarRect.GetWidth()) - tab_bar->ScrollingAnim, 0.f);
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    bool want_clip_rect = !avail_bar_rect.Contains(ImRect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(scrolling_buttons_width, 0.f)));
    if (want_clip_rect)
        PushClipRect(tab_bar->BarRect.Min, tab_bar->BarRect.Max + ImVec2(g.Style.ItemInnerSpacing.x, 0.f), true);
    ...
    tab_bar->BarRect.Max.x -= scrolling_buttons_width + 1.f;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // Layout
    size.x = tab->Width;
    window->DC.CursorPos = tab_bar->BarRect.Min + ImVec2((float)(int)tab->Offset - tab_bar->ScrollingAnim, 0.f);
    ImVec2 pos = window->DC.CursorPos;
    ...
    if (held && !tab_appearing && IsMouseDragging(0))
    {
        if (!g.DragDropActive && (tab_bar->Flags & ImGuiTabBarFlags_Reorderable))
        {
            ...
            if (g.IO.MouseDelta.x < 0.f && g.IO.MousePos.x < bb.Min.x)
            {
                if (tab_bar->Flags & ImGuiTabBarFlags_Reorderable)
                    TabBarQueueChangeTabOrder(tab_bar, tab, -1);
            }
            else if (g.IO.MouseDelta.x > 0.f && g.IO.MousePos.x > bb.Max.x)
            {
                if (tab_bar->Flags & ImGuiTabBarFlags_Reorderable)
                    TabBarQueueChangeTabOrder(tab_bar, tab, +1);
            }
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ...
    if (has_close_button)
        ...
        size.x += g.Style.FramePadding.x + 1.f;
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    IM_UNUSED(flags);
    IM_ASSERT(width > 0.f);
    const float rounding = ImMax(0.f, ImMin(g.Style.TabRounding, width * 0.5f - 1.f));
    const float y1 = bb.Min.y + 1.f;
    const float y2 = bb.Max.y - 1.f;
    draw_list->PathLineTo(ImVec2(bb.Min.x, y2));
    draw_list->PathArcToFast(ImVec2(bb.Min.x + rounding, y1 + rounding), rounding, 6, 9);
    draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding, y1 + rounding), rounding, 9, 12);
    draw_list->PathLineTo(ImVec2(bb.Max.x, y2));
    draw_list->PathFillConvex(col);
    if (g.Style.TabBorderSize > 0.f)
    {
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id)
{
    ...
    if (label_size.x > text_ellipsis_clip_bb.GetWidth())
    {
        ...
        const float ellipsis_width = (1.f + 1.f) * ellipsis_dot_count - 1.f;
        const char* label_end = NULL;
        float label_size_clipped_x = g.Font->CalcTextSizeA(g.FontSize, text_ellipsis_clip_bb.GetWidth() - ellipsis_width + 1.f, 0.f, label, label_display_end, &label_end).x;
        if (label_end == label && label_end < label_display_end)    // Always display at least 1 character if there's no room for character + ellipsis
        {
            ...
            label_size_clipped_x = g.Font->CalcTextSizeA(g.FontSize, FLT_MAX, 0.f, label, label_end).x;
        }
        while (label_end > label && ImCharIsBlankA(label_end[-1])) // Trim trailing space
        {
            label_end--;
            label_size_clipped_x -= g.Font->CalcTextSizeA(g.FontSize, FLT_MAX, 0.f, label_end, label_end + 1).x; // Ascii blanks are always 1 byte
        }
        RenderTextClippedEx(draw_list, text_pixel_clip_bb.Min, text_pixel_clip_bb.Max, label, label_end, &label_size, ImVec2(0.f, 0.f));

        const float ellipsis_x = text_pixel_clip_bb.Min.x + label_size_clipped_x + 1.f;
    ...
    {
        RenderTextClippedEx(draw_list, text_pixel_clip_bb.Min, text_pixel_clip_bb.Max, label, label_display_end, &label_size, ImVec2(0.f, 0.f));
    }
